...
object IslandGeoDataAnalyzeTest {
  ...
  def analyzeTreesCrownAreaInSection(dataSliceSparkAccessor:DataSliceSparkAccessor):Unit = {
    ...
    val caculRes = mergedSectionStaticResultDF.select("REALMGLOBALUID","sum(SGMJ)","BKMC","GNQHID","TGDA_GS_GLGEOMETRYCONTENT","BH","SHAPE_AREA","avg(SGMJ)")
  ...
  def analyzeTreesMaintainNozzlesEffect(dataSliceSparkAccessor:DataSliceSparkAccessor):Unit = {
    ...
    val mockTreesMaintainNozzleDF = individualTreeDF.sample(0.01).select("REALMGLOBALUID","DMID","TGDA_GS_GLGEOMETRYCONTENT","OBJECTID_1","geoLocation")
