package com.github.tgda.dataAnalyze.util.spark

import com.github.tgda.dataAnalyze.util.coreRealm.GeospatialScaleLevel
import com.github.tgda.dataAnalyze.util.coreRealm.GeospatialScaleLevel.GeospatialScaleLevel
    ...
    geospatialLevel match {
      case GeospatialScaleLevel.GlobalLevel =>
        if(spatialAttributeName == null){
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_GLGeometryContent) AS GL_Geometry FROM "+dataSliceName
        }else{
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_GLGeometryContent) AS "+spatialAttributeName+" FROM "+dataSliceName
        }
      case GeospatialScaleLevel.CountryLevel =>
        if(spatialAttributeName == null){
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_CLGeometryContent) AS CL_Geometry FROM "+dataSliceName
        }else{
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_CLGeometryContent) AS "+spatialAttributeName+" FROM "+dataSliceName
        }
      case GeospatialScaleLevel.LocalLevel =>
        if(spatialAttributeName == null){
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_LLGeometryContent) AS LL_Geometry FROM "+dataSliceName
        }else{
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_LLGeometryContent) AS "+spatialAttributeName+" FROM "+dataSliceName
        }
    }
