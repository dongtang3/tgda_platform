    ...
    geoSpatialLevel match {
      case GlobalLevel =>
        if(spatialAttributeName == null){
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_GLGeometryContent) AS GL_Geometry FROM "+dataSliceName+ " WHERE TGDA_GS_GLGeometryContent IS NOT NULL"
        }else{
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_GLGeometryContent) AS "+spatialAttributeName+" FROM "+dataSliceName + " WHERE "+spatialAttributeName+" IS NOT NULL"
        }
      case CountryLevel =>
        if(spatialAttributeName == null){
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_CLGeometryContent) AS CL_Geometry FROM "+dataSliceName+ " WHERE TGDA_GS_CLGeometryContent IS NOT NULL"
        }else{
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_CLGeometryContent) AS "+spatialAttributeName+" FROM "+dataSliceName + " WHERE "+spatialAttributeName+" IS NOT NULL"
        }
      case LocalLevel =>
        if(spatialAttributeName == null){
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_LLGeometryContent) AS LL_Geometry FROM "+dataSliceName+ " WHERE TGDA_GS_LLGeometryContent IS NOT NULL"
        }else{
          spatialConvertSQL = "SELECT * , ST_GeomFromWKT(TGDA_GS_LLGeometryContent) AS "+spatialAttributeName+" FROM "+dataSliceName + " WHERE "+spatialAttributeName+" IS NOT NULL"
        }
    }
    val orgDataFrame = getDataFrameFromDataSlice(dataSliceName,sliceGroup)
    orgDataFrame.createOrReplaceTempView(dataSliceName)
