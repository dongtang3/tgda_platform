...
import com.github.tgda.engine.core.internal.neo4j.dataTransformer.GetListRelationshipAttachTransformer;
...
public class CommonOperationUtil {
    ...
    public static void generateEntityMetaAttributes(Map<String,Object> propertiesMap){
        if(propertiesMap != null) {
            ...
            propertiesMap.put(Constant._createDateProperty, currentDateTime);
            propertiesMap.put(Constant._lastModifyDateProperty, currentDateTime);
            propertiesMap.put(Constant._dataOriginProperty, COMMON_DATA_ORIGIN);
        }
    }
    ...
    public static List<String> clearSystemBuiltinAttributeNames(List<String> attributeNameList){
        ...
        if(attributeNameList != null){
            for(String currentName:attributeNameList){
                if(!currentName.equals(Constant._createDateProperty)&&
                        !currentName.equals(Constant._lastModifyDateProperty)&&
                        !currentName.equals(Constant._dataOriginProperty)){
    ...
    public static void updateEntityMetaAttributes(GraphOperationExecutor workingGraphOperationExecutor,String entityUID,boolean isRelationshipEntity){
        if (entityUID != null) {
            Map<String,Object> metaAttributesMap = new HashMap<>();
            metaAttributesMap.put(Constant._lastModifyDateProperty,new Date());
    ...
    public static void attachEntities(String entityKind,List<String> entityUIDList,GraphOperationExecutor workingGraphOperationExecutor){
        try {
            ...
            queryParameters.setDefaultFilteringItem(new EqualFilteringItem(Constant._relationAttachSourceKind,entityKind));
            queryParameters.addFilteringItem(new EqualFilteringItem(Constant._relationAttachTargetKind,entityKind), QueryParameters.FilteringLogic.OR);
            String queryCql = CypherBuilder.matchNodesWithQueryParameters(Constant.RelationAttachKindClass,queryParameters,null);
            GetListRelationshipAttachTransformer getListRelationAttachKindTransformer = new GetListRelationshipAttachTransformer(null,workingGraphOperationExecutor);
            Object relationAttachKindsRes = workingGraphOperationExecutor.executeWrite(getListRelationAttachKindTransformer,queryCql);
            List<RelationshipAttach> relationshipAttachList = relationAttachKindsRes != null ? (List<RelationshipAttach>) relationAttachKindsRes : null;

            for(RelationshipAttach currentRelationshipAttach : relationshipAttachList){
                Neo4JRelationshipAttachImpl neo4JRelationAttachKindImpl = (Neo4JRelationshipAttachImpl) currentRelationshipAttach;
                ...
                if(sourceKindName.equals(entityKind)){
                    if(entityUIDList.size() ==1){
                        neo4JRelationAttachKindImpl.newRelationEntities(entityUIDList.get(0), RelationshipAttach.EntityRelateRole.SOURCE,null);
                    }else{
                        neo4JRelationAttachKindImpl.newRelationEntities(entityUIDList, RelationshipAttach.EntityRelateRole.SOURCE,null);
                    }
                }
                if(targetKindName.equals(entityKind)){
                    if(entityUIDList.size() ==1){
                        neo4JRelationAttachKindImpl.newRelationEntities(entityUIDList.get(0), RelationshipAttach.EntityRelateRole.TARGET,null);
                    }else{
                        neo4JRelationAttachKindImpl.newRelationEntities(entityUIDList, RelationshipAttach.EntityRelateRole.TARGET,null);
                    }
                }
            }
