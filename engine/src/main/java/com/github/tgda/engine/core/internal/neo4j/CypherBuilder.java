...
import com.github.tgda.engine.core.analysis.query.*;
...
import static com.github.tgda.engine.core.internal.neo4j.CypherBuilder.CypherFunctionType.EXISTS;
import static com.github.tgda.engine.core.internal.neo4j.CypherBuilder.ReturnRelationableDataType.NODE;
import static com.github.tgda.engine.core.internal.neo4j.CypherBuilder.ReturnRelationableDataType.RELATIONSHIP;
...
public class CypherBuilder {
    ...
    public enum ReturnRelationableDataType{
        BOTH, RELATIONSHIP,NODE,COUNT_NODE, COUNT_RELATIONSHIP
    }
    ...
    public static String matchRelatedNodesFromSpecialStartNodes(CypherFunctionType sourcePropertyFunctionType, Object sourcePropertyValue,
                                                                ...
                                                                Direction direction, CypherFunctionType returnFunctionType) {
        Node sourceNode = Cypher.anyNode().named(sourceNodeName);
    ...
    public static String matchRelatedNodeAndRelationPairsFromSpecialStartNodes(CypherFunctionType sourcePropertyFunctionType, Object sourcePropertyValue,
                                                                String targetConceptionKind, String relationKind, Direction direction) {
        Node sourceNode = Cypher.anyNode().named(sourceNodeName);
        Node resultNodes = Cypher.node(targetConceptionKind).named(operationResultName);
        StatementBuilder.OngoingReadingWithoutWhere ongoingReadingWithoutWhere = null;
    ...
    public static String matchRelatedNodesFromSpecialStartNodes(CypherFunctionType sourcePropertyFunctionType, Object sourcePropertyValue, QueryParameters targetConceptionKindQueryParameters,
                                                                String relationKind, Direction direction) throws EngineServiceEntityExploreException {
        Node sourceNode = Cypher.anyNode().named(sourceNodeName);
        ...
        if (targetConceptionKindQueryParameters != null) {
            ...
            if (ongoingReadingWithWhere != null) {
                ongoingReadingWithWhere.
                        and(resultNodes.hasLabels(Constant.ConceptionKindClass).not()).
                        and(resultNodes.hasLabels(Constant.AttributesViewKindClass).not()).
                        and(resultNodes.hasLabels(Constant.AttributeClass).not()).
                        and(resultNodes.hasLabels(Constant.RelationKindClass).not()).
                        and(resultNodes.hasLabels(Constant.MetaConfigItemsStorageClass).not()).
                        and(resultNodes.hasLabels(Constant.ClassificationClass).not());
                statement = ongoingReadingWithWhere.returning(resultNodes,sourceNode).build();
            } else {
                ongoingReadingWithoutWhere.
                        where(resultNodes.hasLabels(Constant.ConceptionKindClass).not()).
                        and(resultNodes.hasLabels(Constant.AttributesViewKindClass).not()).
                        and(resultNodes.hasLabels(Constant.AttributeClass).not()).
                        and(resultNodes.hasLabels(Constant.RelationKindClass).not()).
                        and(resultNodes.hasLabels(Constant.MetaConfigItemsStorageClass).not()).
    ...
    public static String match2JumpRelatedNodesFromSpecialStartNodes(CypherFunctionType sourcePropertyFunctionType, Object sourcePropertyValue,
                                                                     String middleConceptionKind, String middleRelationKind, Direction middleDirection,
                                                                     Map<String, Object> middleEntityProperties,
                                                                     String targetConceptionKind, String targetRelationKind, Direction targetDirection,
    ...
    public static String matchNodesWithQueryParameters(String labelName, QueryParameters queryParameters, CypherFunctionType cypherFunctionType) throws EngineServiceEntityExploreException {
        int defaultReturnRecordNumber = 10000;
        Node m = null;
        Statement statement = null;
        if (labelName != null) {
    ...
    public static String matchAttributesWithQueryParameters(String labelName, QueryParameters queryParameters,List<String> returnProperties) throws EngineServiceEntityExploreException {
        int defaultReturnRecordNumber = 10000;
        Node m = null;
        Statement statement = null;
        if (labelName != null) {
    ...
    public static String matchRelatedNodesAndRelationsFromSpecialStartNodes(CypherFunctionType sourcePropertyFunctionType, Object sourcePropertyValue,
                                                                            String targetConceptionKind, String relationKind,
                                                                            Direction direction, int minJump, int maxJump, ReturnRelationableDataType returnRelationableDataType) {
        Node sourceNode = Cypher.anyNode().named(sourceNodeName);
    ...
    public static String matchRelatedNodesAndRelationsFromSpecialStartNodes(CypherFunctionType sourcePropertyFunctionType, Object sourcePropertyValue,
                                                                            String targetConceptionKind, String relationKind,
                                                                            Direction direction, int minJump, int maxJump,
                                                                            AttributesParameters relationAttributesParameters, AttributesParameters conceptionAttributesParameters,
                                                                            ResultEntitiesParameters resultEntitiesParameters, ReturnRelationableDataType returnRelationableDataType) throws EngineServiceEntityExploreException {
        Node sourceNode = Cypher.anyNode().named(sourceNodeName);
        Node resultNodes = targetConceptionKind != null ? Cypher.node(targetConceptionKind).named(operationResultName):Cypher.anyNode().named(operationResultName);
        StatementBuilder.OngoingReadingWithoutWhere ongoingReadingWithoutWhere = null;
        StatementBuilder.OngoingReadingWithWhere ongoingReadingWithWhere = null;
        ...
        if(resultEntitiesParameters != null){
            ...
            if ((sortingItemList.size() > 0 && returnRelationableDataType.equals(NODE)) || (sortingItemList.size() > 0 && returnRelationableDataType.equals(RELATIONSHIP))){
                sortItemArray = new SortItem[sortingItemList.size()];
                for (int i = 0; i < sortingItemList.size(); i++) {
                    SortingItem currentSortingItem = sortingItemList.get(i);
                    String attributeName = currentSortingItem.getAttributeName();
    ...
    public static String matchRelationshipsWithQueryParameters(CypherFunctionType nodePropertyFunctionType,
                                                               ...
                                                               QueryParameters relationshipQueryParameters, CypherFunctionType returnFunctionType) throws EngineServiceEntityExploreException {
    ...
    public static String statistNodesWithQueryParametersAndStatisticFunctions(String labelName, QueryParameters queryParameters, List<NumericalAttributeStatisticCondition> statisticConditions, String groupByProperty) throws EngineServiceEntityExploreException {
        Node m = null;
        Statement statement = null;
        if (labelName != null) {
            m = Cypher.node(labelName).named(operationResultName);
    ...
    public static String statistRelationsWithQueryParametersAndStatisticFunctions(String typeName, QueryParameters queryParameters, List<NumericalAttributeStatisticCondition> statisticConditions, String groupByProperty) throws EngineServiceEntityExploreException {
        Node sourceNode = Cypher.anyNode().named(sourceNodeName);
        Node targetNode = Cypher.anyNode().named(targetNodeName);
        Relationship relations;
        Statement statement = null;
    ...
    */
    public static String generateRelationKindMatchLogicsQuery(List<RelationKindMatchLogic> relationKindMatchLogics, Direction defaultDirectionForNoneRelationKindMatch){
        ...
        if(relationKindMatchLogics != null && relationKindMatchLogics.size()>0){
            ...
            for(RelationKindMatchLogic currentRelationKindMatchLogic:relationKindMatchLogics){
                ...
                if(currentRelationKindName != null){
                    ...
                    switch(currentRelationKindMatchLogic.getDirection()){
    ...
    */
    ...
    public static String generateAttributesParametersQueryLogic(AttributesParameters attributesParameters,String entityNodeName) throws EngineServiceEntityExploreException {
    ...
    public static String matchNodeWithSpecialRelationAndAttributeFilter(String relationKind, Direction direction, String originalConceptionKind, List<String> originalEntityUIDS, String aimConceptionKind, QueryParameters targetConceptionKindQueryParameters) throws EngineServiceEntityExploreException {

        List<Long> originalConceptionEntitiesUIDList = null;
        if(originalEntityUIDS != null){
            originalConceptionEntitiesUIDList = new ArrayList<>();
            for(int i=0;i<originalEntityUIDS.size();i++){
                originalConceptionEntitiesUIDList.add(Long.parseLong(originalEntityUIDS.get(i)));
            }
        }
    ...
    public static String matchAttributesWithNodeIDs(List<String> nodeIdList,List<String> returnProperties) throws EngineServiceEntityExploreException {
    ...
    public static String matchRelationsWithUIDs(List<String> relationIdList) throws EngineServiceEntityExploreException {
        List<Long> nodeIdValueList = new ArrayList<>();
