...
public class TimeFlowTest {
    ...
    @BeforeTest
    public void initData() throws EngineServiceRuntimeException {
        System.out.println("--------------------------------------------------");
    ...
    private void setupTimeFlowFunction() throws EngineServiceRuntimeException {
    ...
    @Test
    public void testTimeFlowFunction() throws EngineServiceRuntimeException, EngineServiceEntityExploreException {
        CoreRealm coreRealm = EngineFactory.getDefaultEngine();
        Assert.assertEquals(coreRealm.getStorageImplTech(), StorageImplTech.NEO4J);
        ...
        newEntityValue.put("prop1",Long.parseLong("12345"));
        newEntityValue.put("prop2",Double.parseDouble("12345.789"));
        EntityValue conceptionEntityValue2 = new EntityValue(newEntityValue2);

        Entity _Entity2 = _ConceptionKind02.newEntity(conceptionEntityValue,false);
        ...
        TimeScaleEvent timeScaleEvent1 = _Entity.attachTimeScaleEvent(eventCalendar.getTimeInMillis(),"timeEventRelation",timeEventDataMap, TimeFlow.TimeScaleGrade.MINUTE);
        ...
        relationAttachInfo.setDirection(Direction.FROM);
        ...
        Assert.assertEquals(timeScaleEvent1.getAttachEntity().getConceptionKindName(),"TimeFlowEventTest");
        Assert.assertEquals(timeScaleEvent1.getAttachEntity().getEntityUID(),_Entity.getEntityUID());

        _Entity.attachFromRelation(_Entity2.getEntityUID(),"timeEventRelation2",null,true);

        List<TimeScaleEvent> attachedTimeScaleEventsList = _Entity.getAttachedTimeScaleEvents();
        if(attachedTimeScaleEventsList.size()>0) {
            ...
            Assert.assertNotNull(attachedTimeScaleEventsList.get(0).getAttachEntity());
            Assert.assertNotNull(attachedTimeScaleEventsList.get(0).getReferTimeScaleEntity());
        }

        List<TimeScaleEntity> attachedTimeScaleEntityList = _Entity.getAttachedTimeScaleEntities();
        ...
        List<TimeScaleDataPair> timeScaleDataPairList = _Entity.getAttachedTimeScaleDataPairs();
        ...
        boolean detachTimeScaleEventRes = _Entity.detachTimeScaleEvent(attachedTimeScaleEventsList.get(0).getTimeScaleEventUID());
        ...
        Entity targetEntity = _ConceptionKind.getEntities(queryParameters).getConceptionEntities().get(0);
        Assert.assertEquals(targetEntity.getAttachedTimeScaleEvents().size(),0);
        LocalDateTime dt = LocalDateTime.parse("2004-06-28T08:41:00");
        TimeScaleEvent resultTimeScaleEvent = targetEntity.attachTimeScaleEvent(dt,"comment",null, TimeFlow.TimeScaleGrade.MONTH);
        ...
        for(TimeScaleEvent currentTimeScaleEvent:timeScaleEventsRetrieveResult.getTimeScaleEvents()){
            ...
            Assert.assertNotNull(currentTimeScaleEvent.getAttachEntity());
